...
**Pytanie:** Define the following Kafka primitives in one sentence each: broker, topic, partition, offset, consumer group.
...
**Pułapka rozmowna:** Interviewers ask "can Kafka guarantee global message ordering?" — the answer is no, only per-partition ordering. If you need total order, you must use a single-partition topic (which kills parallelism) or encode ordering into the business logic.

**Tagi:** kafka, fundamentals, topic, partition, consumer-group

---

## Q-KH-002 [bloom: recall]
**Pytanie:** How does Kafka decide which partition a producer message lands in?
...
**Pułapka rozmowna:** "Does adding more partitions after the fact break key routing?" — yes. Because the number of partitions changes, `hash(key) % numPartitions` resolves to a different partition for existing keys. Hot data (e.g., a popular product ID) will silently move. Repartitioning must be planned carefully.

**Tagi:** kafka, partitioning, producer, key

---

## Q-KH-003 [bloom: recall]
**Pytanie:** What is a consumer group and why is the number of active consumers bounded by the partition count?
...
**Pułapka rozmowna:** "Can two consumers in the same group read the same message?" — No, that's the point of a group. If you need fan-out (every service reads every message), each service must use a **different consumer group**.

**Tagi:** kafka, consumer-group, parallelism, scaling

---

## Q-KH-004 [bloom: recall]
**Pytanie:** What is the difference between committed offset and current position in a Kafka consumer, and what is the risk of auto-commit?
...
**Pułapka rozmowna:** "If I set auto-commit to false and crash mid-batch, do I lose data?" — No, records are redelivered. The danger is the *opposite*: if your processing is not idempotent, you will process them twice.

**Tagi:** kafka, offset, auto-commit, at-least-once

---

## Q-KH-005 [bloom: recall]
**Pytanie:** Explain replication factor and ISR (in-sync replicas) in Kafka.
...
**Pułapka rozmowna:** "`acks=all` does NOT mean all physical replicas acknowledged — it means all *currently ISR* replicas acknowledged." This distinction breaks a lot of "I thought it was safe" assumptions.

**Tagi:** kafka, replication, ISR, durability

---

## Q-KH-006 [bloom: recall]
**Pytanie:** What do producer acknowledgment modes `acks=0`, `acks=1`, and `acks=all` mean? What do you trade in each case?
...
**Pułapka rozmowna:** "Can I lose data with `acks=all`?" — Yes, if `min.insync.replicas=1` (default), a single-broker ISR means `acks=all` degrades to `acks=1` semantics.

**Tagi:** kafka, producer, acks, durability

---

## Q-KH-007 [bloom: recall]
**Pytanie:** What is a compacted topic in Kafka and when would you use one?
...
**Pułapka rozmowna:** "Does compaction happen immediately?" — No. Compaction runs asynchronously in the background. There is always a "dirty" section of uncompacted records. Your consumer may see multiple records for the same key; always handle the latest-wins logic.

**Tagi:** kafka, compaction, cleanup-policy, log-compaction

---

## Q-KH-008 [bloom: recall]
**Pytanie:** Describe what Hermes is in one paragraph and name its three main components.
...
**Pułapka rozmowna:** "Is Hermes a replacement for Kafka?" — No. Kafka is still the durable store underneath. Hermes is an abstraction layer and operational platform on top of it.

**Tagi:** hermes, architecture, components, fundamentals

---

## Q-KH-009 [bloom: recall]
**Pytanie:** When would you use Avro instead of JSON in Hermes, and what role does the schema registry play?
...
**Pułapka rozmowna:** "Can you publish JSON to an Avro topic?" — No. Hermes enforces the topic's declared content type. You'll get a 400 at the Frontend.

**Tagi:** hermes, avro, json, schema-registry

---

## Q-KH-010 [bloom: recall]
**Pytanie:** What delivery semantics does Hermes provide and why can retries cause duplicate processing?
...
**Pułapka rozmowna:** "Can Hermes guarantee exactly-once?" — No, and be suspicious of any system claiming exactly-once across a network boundary without 2PC. Even Kafka's transactional exactly-once is scoped to within-Kafka reads and writes, not to external systems.

**Tagi:** hermes, delivery-semantics, at-least-once, idempotency, retries

---

## Q-KH-011 [bloom: understand]
**Pytanie:** Explain idempotent producer and transactional producer in Kafka. What does each give you and where do their guarantees stop?
...
**Pułapka rozmowna:** "Does `acks=all` + idempotent producer give you exactly-once end-to-end?" — No. Exactly-once to an external system requires idempotent write logic on the consumer side. Kafka's guarantees are Kafka-internal only.

**Tagi:** kafka, idempotent-producer, transactions, exactly-once

---

## Q-KH-012 [bloom: understand]
**Pytanie:** Why do partitions exist? Explain both the throughput scaling argument and the ordering guarantee they provide.
...
**Pułapka rozmowna:** "Can I have ordering across multiple users on the same partition?" — Yes, but inter-user ordering is incidental, not guaranteed by Kafka contract. Only intra-key ordering is guaranteed.

**Tagi:** kafka, partition, ordering, throughput, scaling

---

## Q-KH-013 [bloom: understand]
**Pytanie:** Compare Hermes push-over-HTTP vs native Kafka pull from the subscriber's perspective. Go deep on the consumer-side operational mechanics — what you gain and what you give up with each model.
...
**Pułapka rozmowna:** "If my Hermes subscriber is slow, does it slow down other subscribers on the same topic?" — In Hermes, each subscription is a separate Kafka consumer group internally, so a slow subscriber does not block others. This is a key operational advantage over a single shared consumer group.

**Tagi:** hermes, kafka, push-vs-pull, consumer-mechanics, back-pressure

---

## Q-KH-014 [bloom: understand]
**Pytanie:** Explain each Hermes subscription policy field and how they interact: `rate`, `inflightSize`, `messageTtl`, `requestTimeout`, `retryClientErrors`, and the backoff strategy.
...
**Pułapka rozmowna:** "What happens if `retryClientErrors=true` and your subscriber has a bug that returns 400 for all messages?" — Hermes retries the message until TTL expires, pounding your subscriber with invalid requests and filling the retry queue. Always set `retryClientErrors=false` for production subscriptions.

**Tagi:** hermes, subscription-policy, rate-limiting, retry, ttl

---

## Q-KH-015 [bloom: understand]
**Pytanie:** How does dead-letter handling work in Hermes? When do messages end up in the DLQ and how do you inspect or replay them?
...
**Pułapka rozmowna:** "Can you replay indefinitely?" — Replaying re-runs through the subscription policy including TTL. If the DLQ message's original TTL has expired, replaying may immediately re-DLQ unless the subscription TTL is extended first or TTL is measured from replay time (check Hermes version behavior).

**Tagi:** hermes, dlq, dead-letter, replay, subscription

---

## Q-KH-016 [bloom: understand]
**Pytanie:** Explain Avro schema evolution: what are backward, forward, and full compatibility? Which changes are safe and which break compatibility?
...
**Pułapka rozmowna:** "Can I rename a field safely?" — Only if you add the old name as an alias in the new schema: `"aliases": ["oldName"]`. Without the alias, it's a breaking change.

**Tagi:** avro, schema-evolution, compatibility, schema-registry

---

## Q-KH-017 [bloom: apply]
**Pytanie:** Write a Kotlin Spring REST controller that acts as a Hermes subscription endpoint. It receives a batch of `UserImpressionEvent` Avro-deserialized objects as JSON, persists them idempotently, and returns the correct HTTP status codes for the Hermes retry contract.
...
**Pułapka rozmowna:** "What if you return `200` but some events in the batch failed?" — Hermes does not support partial batch ACK. The entire batch is either delivered (`200`) or retried. Design your idempotency so that partial success is safe to retry: skip already-processed events, process new ones.

**Tagi:** hermes, kotlin, spring, subscriber, idempotency, http-contract

---

## Q-KH-018 [bloom: apply]
**Pytanie:** You need to configure a Hermes subscription for a high-volume analytics ingest: 5,000 messages per second, the downstream system can tolerate up to 1 hour of delivery delay, and 4xx responses from the subscriber must NOT trigger retries. Show the subscription policy fields you would set and explain each choice.
...
**Pułapka rozmowna:** "Why not set `rate` to exactly 5000?" — Because Hermes enforces the rate limit strictly. Any burst spike above 5000 is throttled. 10% headroom absorbs natural traffic variance.

**Tagi:** hermes, subscription-policy, high-throughput, analytics, configuration

---

## Q-KH-019 [bloom: apply]
**Pytanie:** Design an idempotency scheme for a Hermes consumer where the business key is `(userId, productId, timestamp)`. Show the schema and insert logic for both PostgreSQL and Cassandra.
...
**Pułapka rozmowna:** "What if two events have the same `(userId, productId)` but different timestamps that collide due to clock skew?" — They are different events. Cassandra treats them as separate rows (different clustering key). PostgreSQL inserts both. Clock skew produces false negatives (failed dedup), not false positives — generally acceptable for analytics.

**Tagi:** hermes, idempotency, postgresql, cassandra, consumer-design

---

## Q-KH-020 [bloom: apply]
**Pytanie:** Write a Kotlin coroutine-friendly function that POSTs an event to Hermes with retry + timeout, returning `Result<Unit>`. Map 4xx to non-retryable failure and 5xx to retryable failure.
...
**Pułapka rozmowna:** "Should you retry on `429 Too Many Requests`?" — Yes, 429 is a transient condition (rate limit), but respect the `Retry-After` header if present. Treating it as 4xx permanent would drop valid messages.

**Tagi:** hermes, kotlin, coroutines, publisher, retry, result

---

## Q-KH-021 [bloom: apply]
**Pytanie:** Below is a Hermes consumer implementation. Identify all the bugs.
...
**Pułapka rozmowna:** "Bug #4 might not crash in tests" — because in tests you often use `runBlocking {}` which creates a coroutine context. The issue surfaces in production Spring MVC where no coroutine dispatcher is present for `@PostMapping` handlers unless explicitly configured.

**Tagi:** hermes, consumer, bugs, idempotency, kotlin, coroutines, spring

---

## Q-KH-022 [bloom: analyze]
**Pytanie:** A colleague proposes replacing Hermes with raw Kafka consumers for the recommendations system's high-throughput impression ingest. Analyze the trade-offs in depth: throughput and latency math, schema management, observability, and what you lose at fleet scale.
...
**Pułapka rozmowna:** "Native Kafka is always faster" — true in theory but for batch analytics the extra 100ms from Hermes is irrelevant. The operational cost of 20 raw Kafka consumer groups monitored by a team of 6 engineers dwarfs the throughput gain.

**Tagi:** hermes, kafka, trade-offs, fleet-scale, recsys, observability

---

## Q-KH-023 [bloom: analyze]
**Pytanie:** Hermes Consumers experience back-pressure when one slow subscriber slows down delivery to others. Analyze the failure mode and how Hermes mitigates it (referencing the concept of dynamic workload balancing).
...
**Pułapka rozmowna:** "Does dynamic workload balancing fix an overloaded subscriber?" — No. It protects *other* subscriptions from being affected. The overloaded subscriber still experiences lag. The fix for an overloaded subscriber is to scale the subscriber, not to adjust Hermes's scheduler.

**Tagi:** hermes, back-pressure, dynamic-workload-balancing, consumers, fleet-scale

---

## Q-KH-024 [bloom: analyze]
**Pytanie:** Compare Hermes to AWS SNS+SQS, Google Cloud Pub/Sub, NATS, and RabbitMQ. Where does Hermes clearly win? Where would you choose a competitor instead?
...
**Pułapka rozmowna:** "Is Hermes production-ready for non-Allegro companies?" — Yes, it's open source and some external companies use it. But its documentation, community, and operational tooling are optimized for Allegro's internal infrastructure. Adopting it externally requires significant investment in runbooks and monitoring.

**Tagi:** hermes, comparison, sns-sqs, pubsub, nats, rabbitmq, trade-offs
